...
@section('title', 'Mój pulpit')

@section('content')
<div class="grid md:grid-cols-3 gap-4 mb-6">
    <div class="bg-white p-4 rounded shadow">
        <div class="text-sm text-gray-500">Moje pojazdy</div>
        <div class="text-3xl font-semibold">{{ $vehicles->count() }}</div>
    </div>
    <div class="bg-white p-4 rounded shadow">
        <div class="text-sm text-gray-500">Koszty w tym miesiącu</div>
        <div class="text-3xl font-semibold">{{ number_format((float) $monthlyCosts, 2, ',', ' ') }} zł</div>
    </div>
    <div class="bg-white p-4 rounded shadow flex items-center justify-between">
        <span>Szybkie akcje</span>
        <div class="space-x-2">
            <a href="{{ route('driver.incidents.create') }}" class="px-3 py-1 bg-red-600 text-white rounded">+ Incydent</a>
            <a href="{{ route('driver.costs.create') }}" class="px-3 py-1 bg-blue-600 text-white rounded">+ Koszt</a>
        </div>
    </div>
</div>

<div class="bg-white p-4 rounded shadow mb-6">
    <h2 class="font-semibold mb-2">Przypisane pojazdy</h2>
    ...
    @empty
        <p class="text-gray-500">Brak przypisanych pojazdów. Skontaktuj się z managerem.</p>
    @endforelse
</div>
